<template>
  <div class="custom-menu-section tw-border-r">
    <div class="tw-w-full">
      <div class="tw-text-center tw-p-4 tw-w-full">
        <p class="grey--text text--darken-1" style="margin-bottom: 0">
          Mevcut Bakiye
        </p>
        <p class="grey--text text--darken-4 tw-font-semibold" style="margin-bottom: 0; font-size: 18px;">
          $ 1324.32
        </p>
        <div class="tw-pt-4">
          <v-btn
            color="primary"
            depressed
            width="100%"
          >
              <span class="tw-font-semibold">
                Bot Oluştur
              </span>
          </v-btn>
        </div>
      </div>
      <v-divider></v-divider>
      <div class="tw-pt-3 tw-pl-4 tw-font-semibold text--darken-1 grey--text">
          <span style="font-size: 12px;">
            Menü
          </span>
      </div>
      <v-list
        dense
        rounded
      >
        <v-list-item
          color="blue"
          link
          to="/"
        >
          <v-list-item-icon>
            <svg fill="none" height="22" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M19.77 11.25H15.73C13.72 11.25 12.75 10.36 12.75 8.52V3.98C12.75 2.14 13.73 1.25 15.73 1.25H19.77C21.78 1.25 22.75 2.14 22.75 3.98V8.51C22.75 10.36 21.77 11.25 19.77 11.25ZM15.73 2.75C14.39 2.75 14.25 3.13 14.25 3.98V8.51C14.25 9.37 14.39 9.74 15.73 9.74H19.77C21.11 9.74 21.25 9.36 21.25 8.51V3.98C21.25 3.12 21.11 2.75 19.77 2.75H15.73Z"
                fill="currentColor"/>
              <path
                d="M19.77 22.75H15.73C13.72 22.75 12.75 21.77 12.75 19.77V15.73C12.75 13.72 13.73 12.75 15.73 12.75H19.77C21.78 12.75 22.75 13.73 22.75 15.73V19.77C22.75 21.77 21.77 22.75 19.77 22.75ZM15.73 14.25C14.55 14.25 14.25 14.55 14.25 15.73V19.77C14.25 20.95 14.55 21.25 15.73 21.25H19.77C20.95 21.25 21.25 20.95 21.25 19.77V15.73C21.25 14.55 20.95 14.25 19.77 14.25H15.73Z"
                fill="currentColor"/>
              <path
                d="M8.27 11.25H4.23C2.22 11.25 1.25 10.36 1.25 8.52V3.98C1.25 2.14 2.23 1.25 4.23 1.25H8.27C10.28 1.25 11.25 2.14 11.25 3.98V8.51C11.25 10.36 10.27 11.25 8.27 11.25ZM4.23 2.75C2.89 2.75 2.75 3.13 2.75 3.98V8.51C2.75 9.37 2.89 9.74 4.23 9.74H8.27C9.61 9.74 9.75 9.36 9.75 8.51V3.98C9.75 3.12 9.61 2.75 8.27 2.75H4.23Z"
                fill="currentColor"/>
              <path
                d="M8.27 22.75H4.23C2.22 22.75 1.25 21.77 1.25 19.77V15.73C1.25 13.72 2.23 12.75 4.23 12.75H8.27C10.28 12.75 11.25 13.73 11.25 15.73V19.77C11.25 21.77 10.27 22.75 8.27 22.75ZM4.23 14.25C3.05 14.25 2.75 14.55 2.75 15.73V19.77C2.75 20.95 3.05 21.25 4.23 21.25H8.27C9.45 21.25 9.75 20.95 9.75 19.77V15.73C9.75 14.55 9.45 14.25 8.27 14.25H4.23Z"
                fill="currentColor"/>
            </svg>
          </v-list-item-icon>

          <v-list-item-content>
            <v-list-item-title>
                <span style="font-size: 13px;">
                  Dashboard
                </span>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        <v-list-item
          color="blue"
          link
          to="/market"
        >
          <v-list-item-icon>
            <svg fill="none" height="22" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M14.8902 18.22H6.10019C5.58019 18.22 5.09018 18.17 4.65018 18.07C4.42018 18.04 4.14019 17.96 3.85019 17.85C2.52019 17.35 0.950195 16.12 0.950195 13.06V7.90997C0.950195 4.63997 2.83019 2.76001 6.10019 2.76001H14.8902C17.6502 2.76001 19.4302 4.07998 19.9102 6.47998C20.0002 6.91998 20.0402 7.38997 20.0402 7.90997V13.06C20.0402 16.35 18.1702 18.22 14.8902 18.22ZM6.11017 4.28003C3.65017 4.28003 2.46017 5.46999 2.46017 7.92999V13.08C2.46017 14.87 3.09019 15.97 4.38019 16.46C4.58019 16.53 4.7702 16.58 4.9502 16.61C5.3302 16.69 5.70017 16.73 6.11017 16.73H14.9002C17.3602 16.73 18.5502 15.54 18.5502 13.08V7.92999C18.5502 7.50999 18.5202 7.13999 18.4502 6.79999C18.1102 5.09999 16.9502 4.28003 14.9002 4.28003H6.11017Z"
                fill="currentColor"/>
              <path
                d="M17.8901 21.22H9.10009C8.25009 21.22 7.49008 21.1 6.84008 20.85C5.37008 20.3 4.41007 19.14 4.08007 17.48C4.03007 17.23 4.11009 16.97 4.29009 16.8C4.47009 16.62 4.7301 16.55 4.9801 16.61C5.3101 16.68 5.68009 16.72 6.10009 16.72H14.8901C17.3501 16.72 18.5401 15.53 18.5401 13.07V7.91997C18.5401 7.49997 18.5101 7.12996 18.4401 6.78996C18.3901 6.53996 18.4701 6.28997 18.6401 6.10997C18.8201 5.92997 19.0701 5.84996 19.3201 5.90996C21.7201 6.39996 23.0401 8.17997 23.0401 10.92V16.07C23.0401 19.35 21.1701 21.22 17.8901 21.22ZM5.9201 18.22C6.2401 18.8 6.72009 19.21 7.38009 19.45C7.86009 19.63 8.44007 19.72 9.11007 19.72H17.9001C20.3601 19.72 21.5501 18.53 21.5501 16.07V10.92C21.5501 9.33997 21.0601 8.28997 20.0501 7.73997C20.0501 7.79997 20.0501 7.85997 20.0501 7.91997V13.07C20.0501 16.34 18.1701 18.22 14.9001 18.22H6.11007C6.04007 18.22 5.9801 18.22 5.9201 18.22Z"
                fill="currentColor"/>
              <path
                d="M10.4999 13.89C8.62988 13.89 7.10986 12.37 7.10986 10.5C7.10986 8.63 8.62988 7.10999 10.4999 7.10999C12.3699 7.10999 13.8899 8.63 13.8899 10.5C13.8899 12.37 12.3699 13.89 10.4999 13.89ZM10.4999 8.60999C9.45988 8.60999 8.60986 9.46 8.60986 10.5C8.60986 11.54 9.45988 12.39 10.4999 12.39C11.5399 12.39 12.3899 11.54 12.3899 10.5C12.3899 9.46 11.5399 8.60999 10.4999 8.60999Z"
                fill="currentColor"/>
              <path
                d="M4.78027 13.4501C4.37027 13.4501 4.03027 13.1101 4.03027 12.7001V8.30005C4.03027 7.89005 4.37027 7.55005 4.78027 7.55005C5.19027 7.55005 5.53027 7.89005 5.53027 8.30005V12.7001C5.53027 13.1101 5.20027 13.4501 4.78027 13.4501Z"
                fill="currentColor"/>
              <path
                d="M16.21 13.4501C15.8 13.4501 15.46 13.1101 15.46 12.7001V8.30005C15.46 7.89005 15.8 7.55005 16.21 7.55005C16.62 7.55005 16.96 7.89005 16.96 8.30005V12.7001C16.96 13.1101 16.63 13.4501 16.21 13.4501Z"
                fill="currentColor"/>
            </svg>
          </v-list-item-icon>

          <v-list-item-content>
            <v-list-item-title>
                <span style="font-size: 13px;">
                  Market
                </span>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        <v-list-item
          color="blue"
          link
          to="/wallet"
        >
          <v-list-item-icon>
            <svg fill="none" height="22" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M13 11.9H7C6.59 11.9 6.25 11.56 6.25 11.15C6.25 10.74 6.59 10.4 7 10.4H13C13.41 10.4 13.75 10.74 13.75 11.15C13.75 11.56 13.41 11.9 13 11.9Z"
                fill="currentColor"/>
              <path
                d="M2 11.9001C1.59 11.9001 1.25 11.5601 1.25 11.1501V6.53009C1.25 4.08009 3.24 2.09009 5.69 2.09009H11.31C13.88 2.09009 15.75 3.80009 15.75 6.15009C15.75 6.56009 15.41 6.90009 15 6.90009C14.59 6.90009 14.25 6.56009 14.25 6.15009C14.25 4.39009 12.72 3.59009 11.31 3.59009H5.69C4.07 3.59009 2.75 4.91009 2.75 6.53009V11.1501C2.75 11.5701 2.41 11.9001 2 11.9001Z"
                fill="currentColor"/>
              <path
                d="M16 21.9H6C3.38 21.9 1.25 19.77 1.25 17.15V10.15C1.25 7.53002 3.38 5.40002 6 5.40002H16C18.62 5.40002 20.75 7.53002 20.75 10.15V11.6C20.75 12.01 20.41 12.35 20 12.35H18.92C18.57 12.35 18.25 12.48 18.02 12.72L18.01 12.73C17.68 13.06 17.55 13.52 17.67 13.97C17.82 14.53 18.41 14.94 19.07 14.94H20C20.41 14.94 20.75 15.28 20.75 15.69V17.14C20.75 19.77 18.62 21.9 16 21.9ZM6 6.90002C4.21 6.90002 2.75 8.36002 2.75 10.15V17.15C2.75 18.94 4.21 20.4 6 20.4H16C17.79 20.4 19.25 18.94 19.25 17.15V16.45H19.07C17.72 16.45 16.55 15.6 16.22 14.37C15.96 13.4 16.24 12.37 16.95 11.67C17.47 11.14 18.17 10.85 18.92 10.85H19.25V10.15C19.25 8.36002 17.79 6.90002 16 6.90002H6Z"
                fill="currentColor"/>
              <path
                d="M20.9699 16.45H19.0399C17.5299 16.45 16.2499 15.33 16.1299 13.89C16.0499 13.06 16.3499 12.25 16.9499 11.66C17.4499 11.14 18.1599 10.85 18.9099 10.85H20.9599C21.9399 10.85 22.7399 11.64 22.7399 12.62V14.68C22.7499 15.66 21.9499 16.45 20.9699 16.45ZM18.9199 12.35C18.5699 12.35 18.2499 12.48 18.0199 12.72C17.7299 13 17.5899 13.38 17.6299 13.76C17.6799 14.42 18.3199 14.95 19.0399 14.95H20.9699C21.1199 14.95 21.2499 14.83 21.2499 14.68V12.62C21.2499 12.47 21.1199 12.35 20.9699 12.35H18.9199Z"
                fill="currentColor"/>
            </svg>

          </v-list-item-icon>

          <v-list-item-content>
            <v-list-item-title>
                <span style="font-size: 13px;">
                  Cüzdan
                </span>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>

        <v-list-item
          color="blue"
          link
          to="/history"
        >
          <v-list-item-icon>
            <svg fill="none" height="22" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M12.0001 22.75C6.80008 22.75 2.58008 18.52 2.58008 13.33C2.58008 11.46 3.13008 9.65001 4.17008 8.09001C4.40008 7.75001 4.87008 7.65001 5.21008 7.88001C5.55008 8.11001 5.65008 8.58001 5.42008 8.92001C4.55008 10.22 4.09008 11.75 4.09008 13.32C4.09008 17.69 7.64008 21.24 12.0101 21.24C16.3801 21.24 19.9301 17.69 19.9301 13.32C19.9301 8.95001 16.3701 5.40001 12.0001 5.40001C11.0801 5.40001 10.1801 5.53001 9.33008 5.79001C8.93008 5.91001 8.51008 5.69001 8.39008 5.29001C8.27008 4.89001 8.49008 4.47001 8.89008 4.35001C9.89008 4.05001 10.9301 3.89001 12.0001 3.89001C17.2001 3.89001 21.4201 8.12001 21.4201 13.31C21.4201 18.5 17.2001 22.75 12.0001 22.75Z"
                fill="currentColor"/>
              <path
                d="M7.87011 6.07C7.70011 6.07 7.52011 6.01 7.38011 5.89C7.06011 5.61 7.03012 5.14 7.30012 4.83L10.1901 1.51C10.4601 1.2 10.9401 1.16 11.2501 1.44C11.5601 1.71 11.5901 2.19 11.3201 2.5L8.43012 5.81C8.28012 5.98 8.07011 6.07 7.87011 6.07Z"
                fill="currentColor"/>
              <path
                d="M11.2401 8.53C11.0901 8.53 10.9301 8.48 10.8001 8.39L7.42008 5.92C7.09008 5.68 7.02008 5.21 7.26008 4.88C7.50008 4.54 7.97008 4.47 8.31008 4.71L11.6801 7.17C12.0101 7.41 12.0901 7.88 11.8401 8.22C11.7001 8.43 11.4701 8.53 11.2401 8.53Z"
                fill="currentColor"/>
            </svg>

          </v-list-item-icon>

          <v-list-item-content>
            <v-list-item-title>
                <span style="font-size: 13px;">
                  Geçmiş İşlemler
                </span>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list>
      <div class="tw-pt-3 tw-pl-4 tw-font-semibold text--darken-1 grey--text">
          <span style="font-size: 12px;">
            Hesap Ayarları
          </span>
      </div>
      <v-list
        dense
        rounded
      >
        <v-list-item
          color="blue"
          link
          to="/settings"
        >
          <v-list-item-icon>
            <svg fill="none" height="22" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M12 15.75C9.93 15.75 8.25 14.07 8.25 12C8.25 9.93 9.93 8.25 12 8.25C14.07 8.25 15.75 9.93 15.75 12C15.75 14.07 14.07 15.75 12 15.75ZM12 9.75C10.76 9.75 9.75 10.76 9.75 12C9.75 13.24 10.76 14.25 12 14.25C13.24 14.25 14.25 13.24 14.25 12C14.25 10.76 13.24 9.75 12 9.75Z"
                fill="currentColor"/>
              <path
                d="M15.21 22.19C15 22.19 14.79 22.16 14.58 22.11C13.96 21.94 13.44 21.55 13.11 21L12.99 20.8C12.4 19.78 11.59 19.78 11 20.8L10.89 20.99C10.56 21.55 10.04 21.95 9.42 22.11C8.79 22.28 8.14 22.19 7.59 21.86L5.87 20.87C5.26 20.52 4.82 19.95 4.63 19.26C4.45 18.57 4.54 17.86 4.89 17.25C5.18 16.74 5.26 16.28 5.09 15.99C4.92 15.7 4.49 15.53 3.9 15.53C2.44 15.53 1.25 14.34 1.25 12.88V11.12C1.25 9.66004 2.44 8.47004 3.9 8.47004C4.49 8.47004 4.92 8.30004 5.09 8.01004C5.26 7.72004 5.19 7.26004 4.89 6.75004C4.54 6.14004 4.45 5.42004 4.63 4.74004C4.81 4.05004 5.25 3.48004 5.87 3.13004L7.6 2.14004C8.73 1.47004 10.22 1.86004 10.9 3.01004L11.02 3.21004C11.61 4.23004 12.42 4.23004 13.01 3.21004L13.12 3.02004C13.8 1.86004 15.29 1.47004 16.43 2.15004L18.15 3.14004C18.76 3.49004 19.2 4.06004 19.39 4.75004C19.57 5.44004 19.48 6.15004 19.13 6.76004C18.84 7.27004 18.76 7.73004 18.93 8.02004C19.1 8.31004 19.53 8.48004 20.12 8.48004C21.58 8.48004 22.77 9.67004 22.77 11.13V12.89C22.77 14.35 21.58 15.54 20.12 15.54C19.53 15.54 19.1 15.71 18.93 16C18.76 16.29 18.83 16.75 19.13 17.26C19.48 17.87 19.58 18.59 19.39 19.27C19.21 19.96 18.77 20.53 18.15 20.88L16.42 21.87C16.04 22.08 15.63 22.19 15.21 22.19ZM12 18.49C12.89 18.49 13.72 19.05 14.29 20.04L14.4 20.23C14.52 20.44 14.72 20.59 14.96 20.65C15.2 20.71 15.44 20.68 15.64 20.56L17.37 19.56C17.63 19.41 17.83 19.16 17.91 18.86C17.99 18.56 17.95 18.25 17.8 17.99C17.23 17.01 17.16 16 17.6 15.23C18.04 14.46 18.95 14.02 20.09 14.02C20.73 14.02 21.24 13.51 21.24 12.87V11.11C21.24 10.48 20.73 9.96004 20.09 9.96004C18.95 9.96004 18.04 9.52004 17.6 8.75004C17.16 7.98004 17.23 6.97004 17.8 5.99004C17.95 5.73004 17.99 5.42004 17.91 5.12004C17.83 4.82004 17.64 4.58004 17.38 4.42004L15.65 3.43004C15.22 3.17004 14.65 3.32004 14.39 3.76004L14.28 3.95004C13.71 4.94004 12.88 5.50004 11.99 5.50004C11.1 5.50004 10.27 4.94004 9.7 3.95004L9.59 3.75004C9.34 3.33004 8.78 3.18004 8.35 3.43004L6.62 4.43004C6.36 4.58004 6.16 4.83004 6.08 5.13004C6 5.43004 6.04 5.74004 6.19 6.00004C6.76 6.98004 6.83 7.99004 6.39 8.76004C5.95 9.53004 5.04 9.97004 3.9 9.97004C3.26 9.97004 2.75 10.48 2.75 11.12V12.88C2.75 13.51 3.26 14.03 3.9 14.03C5.04 14.03 5.95 14.47 6.39 15.24C6.83 16.01 6.76 17.02 6.19 18C6.04 18.26 6 18.57 6.08 18.87C6.16 19.17 6.35 19.41 6.61 19.57L8.34 20.56C8.55 20.69 8.8 20.72 9.03 20.66C9.27 20.6 9.47 20.44 9.6 20.23L9.71 20.04C10.28 19.06 11.11 18.49 12 18.49Z"
                fill="currentColor"/>
            </svg>

          </v-list-item-icon>

          <v-list-item-content>
            <v-list-item-title>
                <span style="font-size: 13px;">
                  Ayarlar
                </span>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        <v-list-item
          color="blue"
          link
          to="/logout"
        >
          <v-list-item-icon>
            <svg fill="none" height="22" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M17.4399 15.37C17.2499 15.37 17.0599 15.3 16.9099 15.15C16.6199 14.86 16.6199 14.38 16.9099 14.09L18.9399 12.06L16.9099 10.03C16.6199 9.74 16.6199 9.26 16.9099 8.97C17.1999 8.68 17.6799 8.68 17.9699 8.97L20.5299 11.53C20.8199 11.82 20.8199 12.3 20.5299 12.59L17.9699 15.15C17.8199 15.3 17.6299 15.37 17.4399 15.37Z"
                fill="currentColor"/>
              <path
                d="M19.9298 12.81H9.75977C9.34977 12.81 9.00977 12.47 9.00977 12.06C9.00977 11.65 9.34977 11.31 9.75977 11.31H19.9298C20.3398 11.31 20.6798 11.65 20.6798 12.06C20.6798 12.47 20.3398 12.81 19.9298 12.81Z"
                fill="currentColor"/>
              <path
                d="M11.7598 20.75C6.60977 20.75 3.00977 17.15 3.00977 12C3.00977 6.85 6.60977 3.25 11.7598 3.25C12.1698 3.25 12.5098 3.59 12.5098 4C12.5098 4.41 12.1698 4.75 11.7598 4.75C7.48977 4.75 4.50977 7.73 4.50977 12C4.50977 16.27 7.48977 19.25 11.7598 19.25C12.1698 19.25 12.5098 19.59 12.5098 20C12.5098 20.41 12.1698 20.75 11.7598 20.75Z"
                fill="currentColor"/>
            </svg>


          </v-list-item-icon>

          <v-list-item-content>
            <v-list-item-title>
                <span style="font-size: 13px;">
                  Çıkış Yap
                </span>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list>
      <div
        class="tw-pt-1 tw-pl-4 tw-font-semibold text--darken-1 grey--text tw-flex tw-items-center tw-justify-between">
          <span style="font-size: 12px;">
            Takip Listesi
          </span>
        <span
          class="tw-p-2 tw-mr-2 tw-cursor-pointer tw-rounded-full tw-transition tw-duration-200 hover:tw-bg-gray-200">
          <svg fill="none" height="22" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg">
<path
  d="M15 22.75H9C3.57 22.75 1.25 20.43 1.25 15V9C1.25 3.57 3.57 1.25 9 1.25H11C11.41 1.25 11.75 1.59 11.75 2C11.75 2.41 11.41 2.75 11 2.75H9C4.39 2.75 2.75 4.39 2.75 9V15C2.75 19.61 4.39 21.25 9 21.25H15C19.61 21.25 21.25 19.61 21.25 15V13C21.25 12.59 21.59 12.25 22 12.25C22.41 12.25 22.75 12.59 22.75 13V15C22.75 20.43 20.43 22.75 15 22.75Z"
  fill="currentColor"/>
<path
  d="M8.50008 17.6901C7.89008 17.6901 7.33008 17.4701 6.92008 17.0701C6.43008 16.5801 6.22008 15.8701 6.33008 15.1201L6.76008 12.1101C6.84008 11.5301 7.22008 10.7801 7.63008 10.3701L15.5101 2.49006C17.5001 0.500059 19.5201 0.500059 21.5101 2.49006C22.6001 3.58006 23.0901 4.69006 22.9901 5.80006C22.9001 6.70006 22.4201 7.58006 21.5101 8.48006L13.6301 16.3601C13.2201 16.7701 12.4701 17.1501 11.8901 17.2301L8.88008 17.6601C8.75008 17.6901 8.62008 17.6901 8.50008 17.6901ZM16.5701 3.55006L8.69008 11.4301C8.50008 11.6201 8.28008 12.0601 8.24008 12.3201L7.81008 15.3301C7.77008 15.6201 7.83008 15.8601 7.98008 16.0101C8.13008 16.1601 8.37008 16.2201 8.66008 16.1801L11.6701 15.7501C11.9301 15.7101 12.3801 15.4901 12.5601 15.3001L20.4401 7.42006C21.0901 6.77006 21.4301 6.19006 21.4801 5.65006C21.5401 5.00006 21.2001 4.31006 20.4401 3.54006C18.8401 1.94006 17.7401 2.39006 16.5701 3.55006Z"
  fill="currentColor"/>
<path
  d="M19.8501 9.83003C19.7801 9.83003 19.7101 9.82003 19.6501 9.80003C17.0201 9.06003 14.9301 6.97003 14.1901 4.34003C14.0801 3.94003 14.3101 3.53003 14.7101 3.41003C15.1101 3.30003 15.5201 3.53003 15.6301 3.93003C16.2301 6.06003 17.9201 7.75003 20.0501 8.35003C20.4501 8.46003 20.6801 8.88003 20.5701 9.28003C20.4801 9.62003 20.1801 9.83003 19.8501 9.83003Z"
  fill="currentColor"/>
</svg>
          </span>
      </div>
      <div class="tw-px-4 tw-pb-1">
        <div v-for="item,index in 5" :key="index" class="tw-inline-block tw-py-1 tw-pr-1">
          <v-chip :style="{fontWeight: '600', fontSize: '14px'}" close color="primary">
            Bitcoin
          </v-chip>
        </div>
      </div>
      <div
        class="tw-pt-1 tw-pl-4 tw-font-semibold text--darken-1 grey--text">
          <span style="font-size: 12px;">
            Mevcut Botlar
          </span>
      </div>
      <div class="tw-px-4 tw-pt-2">
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-hover>
              <div slot-scope="{hover}" class="tw-flex hover:tw-shadow tw-items-center tw-justify-between tw-transition tw-duration-200 tw-p-2 tw-rounded-lg tw-cursor-pointer" :class="{'grey lighten-2' : hover, 'grey lighten-4' : !hover}"
                   style="font-size: 13px;"
                   v-bind="attrs"
                   v-on="on">
                <div class="tw-flex tw-flex-col">
            <span class="tw-text-center tw-font-semibold grey--text text--darken-4">
              Spot
            </span>
                  <span>
              ETH / USDT
            </span>
                </div>
                <div class="tw-flex tw-flex-col">
                  <span class="tw-text-center tw-font-semibold grey--text text--darken-4">Tür</span>
                  <span>Alım - Satım</span>
                </div>
                <div class="tw-flex tw-flex-col">
                  <span class="tw-text-center tw-font-semibold grey--text text--darken-4">Bakiye</span>
                  <span>$ 325.32</span>
                </div>
              </div>
            </v-hover>
          </template>
          <span>Bot detaylarını görüntüle</span>
        </v-tooltip>

      </div>
    </div>
  </div>
</template>

<script>
import Vue from 'vue';

export default Vue.extend({
  name: "CustomMenu",
  data() {
    return {
      show: false,
      items: [
        {title: 'Home', icon: 'mdi-view-dashboard'},
        {title: 'About', icon: 'mdi-forum'},
      ],
    }
  },
  props: {
    visible: {
      type: Boolean,
      default: false
    }
  },
  watch: {
    visible: {
      handler(newValue) {
        this.show = newValue;
      },
      immediate: true
    },
    show: {
      handler(newValue) {
        if (!newValue) {
          this.$emit('menu_closed')
        }
      }
    }
  },
})
</script>

<style lang="scss" scoped>
.link-container {
  .active {
    background-color: #F0EDFD;

    span {
      color: #3C1DD7 !important;

      svg {
        path {
          fill: #3C1DD7;
        }
      }
    }
  }
}

.custom-menu-section {
  position: fixed;
  left: 0;
  top: 0;
  height: 100%;
  overflow-x: auto;
  width: inherit;
  background-color: white;
}
</style>
